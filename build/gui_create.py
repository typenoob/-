import a_init
import os
import sys
from tkinter.constants import CENTER, TOP
from tkinter import Tk, Canvas, Entry, Text, Button, PhotoImage, font, filedialog
from tkinter.colorchooser import *
from pathlib import Path
import tkinter
from src import pen
from src import file
import gui_main
# This file was generated by the Tkinter Designer by Parth Jadhav
# https://github.com/ParthJadhav/Tkinter-Designer


# from tkinter import *
# Explicit imports to satisfy Flake8
def jump_main(canvas, root):
    canvas.destroy()
    canvas = Canvas(
        root,
        bg="#FFFFFF",
        height=1024,
        width=1440,
        bd=0,
        highlightthickness=0,
        relief="ridge"
    )
    gui_main.entry(canvas, root)


def drawText(event):
    global text, size
    text = tkinter.simpledialog.askstring(title='输入文本', prompt='')
    if text != None:
        size = tkinter.simpledialog.askinteger(
            '输入字号', prompt='', initialvalue=20)  # 默认值为20
        if size == None:
            size = "20"
    draw.create_text(event.x, event.y, font=(
        "微软雅黑", int(size)), text=text, fill=fgcolor)


def save_query():
    Filepath = filedialog.asksaveasfile()
    print(Filepath.name)
    file.save(window, draw, Filepath.name)


def color(f="black"):
    c = askcolor(color=f, title="选择画笔颜色")
    global fgcolor
    fgcolor = c[1]
    pick()
    return fgcolor


def color2(f=""):
    c = askcolor(color=f, title="选择填充颜色")
    global fillf
    fillf = c[1]
    return fillf


def thickadd(t):
    global thick
    thick = t+1
    round()
    return thick


def thickreduce(t):
    global thick
    thick = t-1
    round()
    return thick

# def choose():
#     global button_1, button_2, button_3, button_4, button_5, button_6, button_7
#     button_1['image'] = button_image_2
#     button_2['image'] = button_image_1
#     button_3['image'] = button_image_1
#     button_4['image'] = button_image_1
#     button_5['image'] = button_image_1
#     button_6['image'] = button_image_1
#     button_7['image'] = button_image_1
#     button_1_ = Button(
#         text="相\n同\n颜\n色\n选\n取",
#         bg="#FDF419",
#         font=("YeonSung Regular", 26 * -1),
#         borderwidth=0,
#         highlightthickness=0,
#         command=lambda: print("button_1 clicked") ,
#         relief="flat"
#     )
#     button_1_.place(x=100.0, y=40.0, width=60.0, height=200.0)
#     button_2_ = Button(
#         text="矩\n形\n选\n取",
#         bg="#FDF419",
#         font=("YeonSung Regular", 26 * -1),
#         borderwidth=0,
#         highlightthickness=0,
#         command=lambda: print("button_1 clicked"),
#         relief="flat",
#     )
#     button_2_.place(x=100.0, y=260.0, width=60.0, height=200.0)
#     button_3_ = Button(
#         text="圆\n形\n选\n取",
#         bg="#FDF419",
#         font=("YeonSung Regular", 26 * -1),
#         borderwidth=0,
#         highlightthickness=0,
#         command=lambda: print("button_1 clicked"),
#         relief="flat"
#     )
#     button_3_.place(x=100.0, y=480.0, width=60.0, height=200.0)


def paint():
    from src import pen
    global draw
    global thick
    global fgcolor
    pen.Pen(thick, fgcolor).bind(draw)
    global button_1, button_2, button_3, button_4, button_5, button_6, button_7
    button_1['image'] = button_image_1
    button_2['image'] = button_image_2
    button_3['image'] = button_image_1
    button_4['image'] = button_image_1
    button_5['image'] = button_image_1
    button_6['image'] = button_image_1
    button_7['image'] = button_image_1
    button_1_ = Button(
        text="画\n笔\n粗\n细"+'\n'+str(thick)+'\n'+'px',
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: round(),
        relief="flat"
    )
    button_1_.place(x=100.0, y=40.0, width=60.0, height=200.0)
    button_2_ = Button(
        text="画\n笔\n颜\n色\n..",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: pick(),
        relief="flat"
    )
    button_2_.place(x=100.0, y=260.0, width=60.0, height=200.0)
    button_3_ = Button(
        text="",
        bg=fgcolor,
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: print("button_1 clicked"),
        relief="flat"
    )
    button_3_.place(x=100.0, y=480.0, width=60.0, height=200.0)


def eraser():
    global draw
    from src import eraser
    global button_1, button_2, button_3, button_4, button_5, button_6, button_7
    button_1['image'] = button_image_1
    button_2['image'] = button_image_1
    button_3['image'] = button_image_2
    button_4['image'] = button_image_1
    button_5['image'] = button_image_1
    button_6['image'] = button_image_1
    button_7['image'] = button_image_1
    button_1_ = Button(
        text="小\n型",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: eraser.Eraser(1).bind(draw),
        relief="flat"
    )
    button_1_.place(x=100.0, y=40.0, width=60.0, height=200.0)
    button_2_ = Button(
        text="大\n型",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: eraser.Eraser(2).bind(draw),
        relief="flat"
    )
    button_2_.place(x=100.0, y=260.0, width=60.0, height=200.0)
    button_3_ = Button(
        text="清\n屏",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: draw.delete("all"),
        relief="flat"
    )
    button_3_.place(x=100.0, y=480.0, width=60.0, height=200.0)


def round():
    global draw
    global thick
    global button_1, button_2, button_3, button_4, button_5, button_6, button_7
    button_1['image'] = button_image_1
    button_2['image'] = button_image_1
    button_3['image'] = button_image_1
    button_4['image'] = button_image_2
    button_5['image'] = button_image_1
    button_6['image'] = button_image_1
    button_7['image'] = button_image_1
    button_1_ = Button(
        text="+",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: thickadd(thick),
        relief="flat"
    )
    button_1_.place(x=100.0, y=40.0, width=60.0, height=200.0)
    button_2_ = Button(
        text="-",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: thickreduce(thick),
        relief="flat"
    )
    button_2_.place(x=100.0, y=260.0, width=60.0, height=200.0)
    button_3_ = Button(
        text=str(thick)+'px',
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: print("button_1 clicked"),
        relief="flat"
    )
    button_3_.place(x=100.0, y=480.0, width=60.0, height=200.0)


def round_():
    global draw
    from src import Shape
    global thick
    global fgcolor
    global fillf
    global button_1, button_2, button_3, button_4, button_5, button_6, button_7
    button_1['image'] = button_image_1
    button_2['image'] = button_image_1
    button_3['image'] = button_image_1
    button_4['image'] = button_image_1
    button_5['image'] = button_image_2
    button_6['image'] = button_image_1
    button_7['image'] = button_image_1
    button_1_ = Button(
        text="直\n线",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: Shape.Shape(thick, fgcolor).bind(draw, 1),
        relief="flat"
    )
    button_1_.place(x=100.0, y=40.0, width=60.0, height=200.0)
    button_2_ = Button(
        text="圆\n形",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: Shape.Shape(thick, fgcolor, fillf).bind(draw, 4),
        relief="flat"
    )
    button_2_.place(x=100.0, y=260.0, width=60.0, height=200.0)
    button_3_ = Button(
        text="矩\n形",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: Shape.Shape(thick, fgcolor, fillf).bind(draw, 3),
        relief="flat"
    )
    button_3_.place(x=100.0, y=480.0, width=60.0, height=200.0)


def pick():
    global fgcolor
    global button_1, button_2, button_3, button_4, button_5, button_6, button_7
    button_1['image'] = button_image_1
    button_2['image'] = button_image_1
    button_3['image'] = button_image_1
    button_4['image'] = button_image_1
    button_5['image'] = button_image_1
    button_6['image'] = button_image_2
    button_7['image'] = button_image_1
    button_1_ = Button(
        text="设\n置\n颜\n色\n",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: color(),
        relief="flat"
    )
    button_1_.place(x=100.0, y=40.0, width=60.0, height=200.0)
    button_2_ = Button(
        text='',
        bg=fgcolor,
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: print("button_1 clicked"),
        relief="flat"
    )
    button_2_.place(x=100.0, y=260.0, width=60.0, height=200.0)
    button_3_ = Button(
        text="填\n充\n颜\n色",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: color2(),
        relief="flat"
    )
    button_3_.place(x=100.0, y=480.0, width=60.0, height=200.0)


def text():
    draw.bind('<ButtonRelease-1>', drawText)
    global button_1, button_2, button_3, button_4, button_5, button_6, button_7
    button_1['image'] = button_image_1
    button_2['image'] = button_image_1
    button_3['image'] = button_image_1
    button_4['image'] = button_image_1
    button_5['image'] = button_image_1
    button_6['image'] = button_image_1
    button_7['image'] = button_image_2
    button_1_ = Button(
        text="横\n排\n文\n字",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: print("button_1 clicked"),
        relief="flat"
    )
    button_1_.place(x=100.0, y=40.0, width=60.0, height=200.0)
    button_2_ = Button(
        text="字\n体\n颜\n色",
        bg='#FDF419',
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: pick(),
        relief="flat"
    )
    button_2_.place(x=100.0, y=260.0, width=60.0, height=200.0)
    button_3_ = Button(
        text="微\n软\n雅\n黑",
        bg="#FDF419",
        font=("YeonSung Regular", 26 * -1),
        borderwidth=0,
        highlightthickness=0,
        command=lambda: print("button_1 clicked"),
        relief="flat"
    )
    button_3_.place(x=100.0, y=480.0, width=60.0, height=200.0)


OUTPUT_PATH = Path(__file__).parent
# OUTPUT_PATH=Path(os.path.dirname(os.path.realpath(sys.executable)))
ASSETS_PATH = OUTPUT_PATH / Path("./assets")


def relative_to_assets(path: str) -> Path:
    return ASSETS_PATH / Path(path)


thick = 3
fgcolor = "black"  # 全局变量画笔颜色 默认为黑色
fillf = ''  # 全局变量填充颜色，默认为透明


def entry(canvas, root):
    global thick, fgcolor, fillf, window
    window = root
    global draw, button_image_1, button_image_2, button_image_3, button_image_4, button_image_5, button_image_6, button_1, button_2
    global button_3, button_4, button_5, button_6, button_7, button_8, button_9, button_10, button_11
    window.geometry("1440x1024")
    window.configure(bg="#E0E0E0")

    # canvas = Canvas(
    #     window,
    #     bg="#E0E0E0",
    #     height=1024,
    #     width=1440,
    #     bd=0,
    #     highlightthickness=0,
    #     relief="ridge"
    # )

    canvas.place(x=0, y=0)

    button_image_1 = PhotoImage(
        file=relative_to_assets("button_4.png"))
    button_1 = Button(
        image=button_image_1,
        borderwidth=0,
        highlightthickness=0,
        command=lambda: print("button_1 clicked"),
        relief="flat"
    )
    # button_image_1 = PhotoImage(
    #     file=relative_to_assets("button_4.png"))
    # button_1 = Button(
    #     image=button_image_1,
    #     font=("YeonSung Regular", 40 * -1),
    #     compound=CENTER,
    #     text="选择",
    #     borderwidth=0,
    #     highlightthickness=0,
    #     command=choose,
    #     relief="flat"
    # )

    button_image_2 = PhotoImage(
        file=relative_to_assets("button_5.png"))
    button_image_3 = PhotoImage(
        file=relative_to_assets("image_3.png"))
    button_image_4 = PhotoImage(
        file=relative_to_assets("image_4.png"))
    button_image_5 = PhotoImage(
        file=relative_to_assets("image_5.png"))
    button_image_6 = PhotoImage(
        file=relative_to_assets("image_6.png"))
    button_2 = Button(
        image=button_image_1,
        font=("YeonSung Regular", 40 * -1),
        compound=CENTER,
        text="画笔",
        borderwidth=0,
        highlightthickness=0,
        command=paint,
        relief="flat"
    )
    button_3 = Button(
        image=button_image_1,
        font=("YeonSung Regular", 40 * -1),
        compound=CENTER,
        text="橡皮",
        borderwidth=0,
        highlightthickness=0,
        command=eraser,
        relief="flat"
    )
    button_4 = Button(
        image=button_image_1,
        font=("YeonSung Regular", 40 * -1),
        compound=CENTER,
        text="粗细",
        borderwidth=0,
        highlightthickness=0,
        command=round,
        relief="flat"
    )
    button_5 = Button(
        image=button_image_1,
        font=("YeonSung Regular", 40 * -1),
        compound=CENTER,
        text="形状",
        borderwidth=0,
        highlightthickness=0,
        command=round_,
        relief="flat"
    )
    button_6 = Button(
        image=button_image_1,
        font=("YeonSung Regular", 40 * -1),
        compound=CENTER,
        text="颜色",
        borderwidth=0,
        highlightthickness=0,
        command=pick,
        relief="flat"
    )
    button_7 = Button(
        image=button_image_1,
        font=("YeonSung Regular", 40 * -1),
        compound=CENTER,
        text="文本",
        borderwidth=0,
        highlightthickness=0,
        command=text,
        relief="flat"
    )
    button_8 = Button(
        image=button_image_3,
        borderwidth=0,
        highlightthickness=0,
        command=lambda: pen.Pen(thick).re(draw, rev=1),
        relief="flat"
    )
    button_9 = Button(
        image=button_image_4,
        borderwidth=0,
        highlightthickness=0,
        command=save_query,
        relief="flat"
    )
    button_10 = Button(
        image=button_image_5,
        borderwidth=0,
        highlightthickness=0,
        command=lambda: pen.Pen(thick, fgcolor).re(draw, rec=1),
        relief="flat"
    )
    button_11 = Button(
        image=button_image_6,
        borderwidth=0,
        highlightthickness=0,
        command=lambda: jump_main(canvas, root),
        relief="flat"
    )
    # button_1.place(
    #     x=100.0,
    #     y=690.0,
    #     width=100.0,
    #     height=100.0
    # )
    button_2.place(
        x=275.0,
        y=690.0,
        width=100.0,
        height=100.0
    )
    button_3.place(
        x=450.0,
        y=690.0,
        width=100.0,
        height=100.0
    )
    button_4.place(
        x=625.0,
        y=690.0,
        width=100.0,
        height=100.0
    )
    button_5.place(
        x=800.0,
        y=690.0,
        width=100.0,
        height=100.0
    )
    button_6.place(
        x=975.0,
        y=690.0,
        width=100.0,
        height=100.0
    )
    button_7.place(
        x=1150.0,
        y=690.0,
        width=100.0,
        height=100.0
    )
    button_8.place(
        x=1250.0,
        y=70.0,
        width=100.0,
        height=100.0
    )
    button_9.place(
        x=1250.0,
        y=230.0,
        width=100.0,
        height=100.0
    )
    button_10.place(
        x=1250.0,
        y=390.0,
        width=100.0,
        height=100.0
    )
    button_11.place(
        x=1250.0,
        y=550.0,
        width=100.0,
        height=100.0
    )
    image_image_2 = PhotoImage(
        file=relative_to_assets("image_2.png"))
    image_2 = canvas.create_image(
        689.0,
        350.0,
        image=image_image_2
    )
    draw = Canvas(
        window,
        bg="#FFFFFF",
        height=635,
        width=900,
        bd=0,
        highlightthickness=0,
        relief="ridge"
    )
    draw.place(
        x=239.0,
        y=30.0
    )
